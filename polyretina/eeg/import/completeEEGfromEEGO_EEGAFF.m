...
b = str2num(block(end-2));
a = str2num(block(end));
if strcmp('P036',subj_name)
    if b > 1
        inter = b-2;
    end
elseif any(strcmp({'P007','P038'},subj_name))
    if b == 1
        inter = 0;
    end
elseif strcmp(subjects_info(sbj).blockInterruption, 'No')
    inter = b-1;
...
for i = 1:2
    ...
    if i == 1
        chans2search = 1:64; % from EEG_cnt
        %segs2search = segs2search1; % from EEG_cnt
    else
        chans2search = 65:128; % from EEG_cnt
