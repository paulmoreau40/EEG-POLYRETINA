function EEG = fillNaNs(EEG, study_config)
subject = study_config.subjects(study_config.current_subject).id;
[EEG, bound_evts] = findBoundaries(EEG, study_config);
events = EEG.event;
    ...
    for b = 1:length(bound_evts)+1
        ...
        while s <= length(NanDetection)
            if NanDetection(s)
                ...
                if last_valid_sample>0 && s-last_valid_sample>1 && s-last_valid_sample<=study_config.maxNans2Replace+1
