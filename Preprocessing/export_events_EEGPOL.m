...
tbl = table(markers.index, markers.time, markers.events(:,1), repmat({NaN}, events_count,1), repmat({NaN}, events_count,1), cellfun(@str2double, markers.events(:,2)), ...
    cellfun(@str2double, markers.events(:,3)), NaN(events_count,1), 'VariableNames', {'Index', 'Time', 'RawName', 'Name', 'TrialType', 'Value', 'Duration','Latency'});
...
for i=1:events_count
    ...
    % if ismember(row.Value, [20, 45,-5]) && (row.Name ~= "BaselineCoarseStart") % familiarisation, training or trial starts/ends
    %     if ~ismember(row.Name,{'FamEmptyEnd','FamSymbolEnd','TrainEnd','TrialEnd'})
    %         angle = row.Value;
    %     end
    %     row.TrialType = {"Angle"+num2str(angle)};
    % elseif ismember(row.Name), {'BaselineStart','BaselineCoarseStart','BaselineCoarseEnd'}
    %     row.TrialType = 'Baseline';
    % else
    %     error('Event name not recognised ?')
    % end
