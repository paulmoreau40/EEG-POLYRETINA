...
for trial = 1:total_num_trials
    ...
    if field_of_view == 20
        EEG_spectrum_20 = EEG_baseline_data.(['P' participant_id]).spectrum(:,:,current_trial);
    elseif field_of_view == 45
        EEG_spectrum_45 = EEG_baseline_data.(['P' participant_id]).spectrum(:,:,current_trial);
    elseif field_of_view == 110
        EEG_spectrum_110 = EEG_baseline_data.(['P' participant_id]).spectrum(:,:,current_trial);
    end    
end
...
for trial = 1:total_num_trials
    ...
    if field_of_view == 20
        EEG_baseline_data.(['P' participant_id]).spectrum(:,:,current_trial) = mean_EEG_spectrum_20;
    elseif field_of_view == 45
        EEG_baseline_data.(['P' participant_id]).spectrum(:,:,current_trial) = mean_EEG_spectrum_45;
    elseif field_of_view == 110
        EEG_baseline_data.(['P' participant_id]).spectrum(:,:,current_trial) = mean_EEG_spectrum_110;
    end
